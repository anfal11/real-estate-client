  ...
  useEffect(() => {
    const fetchAddedProperties = async () => {
      try {
        ...
        const filteredProperties = response.data.filter(property => property.agentEmail === users[0]?.email);
    ...
    <div className="max-w-7xl mx-auto p-4">
      ...
      {addedProperties.map((property) => (
        <div key={property._id} className="mb-8 border p-4 rounded-md">
          ...
          />
              {
            property?.status === 'For Sale' ? <p className="mt-3 mb-3 text-gray-600 font-extrabold bg-yellow-400 w-24 p-4 text-center rounded-e-full">
          {property?.status}
        </p> : <p className="mt-3 mb-3 text-white font-bold bg-blue-600 w-24 p-4 text-center rounded-e-full">
            {property?.status}
            </p>
        }
          <h2 className="text-xl font-bold mb-2">{property?.title}</h2>
          <p className="text-blue-600 font-medium mb-2">{property?.location}</p>
          <p className="text-gray-600 mb-2">{property?.description}</p>
          <p className="text-gray-600 font-bold mb-2">Agent: {property?.agentName}</p>
          {property.verification === 'verified' ? (
            <p className="text-green-500 font-bold mb-2">Verification Status: Verified</p>
          ) : (
